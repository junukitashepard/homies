...
scale_elhe.none <- function(c, y, secondary = TRUE, name = 'secondary', domestic = TRUE) {
    ...
    elhe2sec6 <- abs(df$value[df$flow == 'Wood and wood products' & df$year == y])
    elhe2sec7 <- sum(abs(df$value[(df$flow == 'Chemical and petrochemical' | df$flow == 'Non-metallic minerals') & df$year == y]))
    elhe2sec8 <- sum(abs(df$value[(df$flow == 'Iron and steel' | df$flow == 'Non-ferrous metals') & df$year == y]))
    elhe2sec9 <- abs(df$value[df$flow == 'Machinery' & df$year == y])
    elhe2sec10 <- abs(df$value[df$flow == 'Transport equipment' & df$year == y])
    elhe2sec11 <- abs(df$value[df$flow == 'Non-specified (industry)' & df$year == y])
    elhe2sec12 <- 0 # Update later
    ...
    elhe2sec16 <- 0 # Update later: trade
    elhe2sec17 <- 0 # Update later: trade
    ...
    elhe2sec25 <- 0
    elhe2sec26 <- 0 # Update later: trade
